...
SELECT u.id, c.id, 'Bếp Sông Quê', 'bep-song-que-vi-tri-demo', 'Cơm nhà, canh nóng và món miền Tây mỗi ngày.', 'Quán demo phục vụ kiểm thử tính năng gợi ý quán ăn theo vị trí hiện tại.', '+84910990001', 'https://images.unsplash.com/photo-1559314809-0d155014e29e?auto=format&fit=crop&w=1400&q=80', 'https://api.dicebear.com/9.x/shapes/svg?seed=BepSongQue&backgroundColor=ffffff', 'Khu vực gần vị trí kiểm thử', NULL, NULL, 'Vĩnh Long', 10.2504000, 105.8682000, 30000, 18, 4.70, 86, 15.00, 1, 'active', NOW(), 1
...
INSERT INTO restaurants (owner_user_id, cuisine_id, name, slug, tagline, description, phone, banner_url, logo_url, address_line, ward, district, city, latitude, longitude, min_order_amount, avg_prep_time_min, rating_avg, review_count, commission_rate, is_open_now, status, approved_at, approved_by_admin_id)
SELECT u.id, c.id, 'Cà Phê Bờ Kênh', 'ca-phe-bo-kenh-vi-tri-demo', 'Cà phê pha máy, bánh ngọt và thức uống mát lạnh.', 'Quán demo phục vụ kiểm thử tính năng gợi ý quán ăn theo vị trí hiện tại.', '+84910990002', 'https://images.unsplash.com/photo-1501339847302-ac426a4a7cbb?auto=format&fit=crop&w=1400&q=80', 'https://api.dicebear.com/9.x/shapes/svg?seed=CaPheBoKenh&backgroundColor=ffffff', 'Khu vực gần vị trí kiểm thử', NULL, NULL, 'Vĩnh Long', 10.2547000, 105.8753000, 25000, 10, 4.60, 54, 12.00, 1, 'active', NOW(), 1
...
INSERT INTO restaurants (owner_user_id, cuisine_id, name, slug, tagline, description, phone, banner_url, logo_url, address_line, ward, district, city, latitude, longitude, min_order_amount, avg_prep_time_min, rating_avg, review_count, commission_rate, is_open_now, status, approved_at, approved_by_admin_id)
SELECT u.id, c.id, 'Ăn Vặt Chợ Chiều', 'an-vat-cho-chieu-vi-tri-demo', 'Món ăn vặt nóng hổi cho buổi xế.', 'Quán demo phục vụ kiểm thử tính năng gợi ý quán ăn theo vị trí hiện tại.', '+84910990003', 'https://images.unsplash.com/photo-1626804475297-41608ea09aeb?auto=format&fit=crop&w=1400&q=80', 'https://api.dicebear.com/9.x/shapes/svg?seed=AnVatChoChieu&backgroundColor=ffffff', 'Khu vực gần vị trí kiểm thử', NULL, NULL, 'Vĩnh Long', 10.2427000, 105.8598000, 20000, 12, 4.50, 39, 12.00, 1, 'active', NOW(), 1
FROM users u JOIN cuisines c ON c.slug = 'vietnamese'
...
JOIN (
  ...
  UNION ALL SELECT 'bep-song-que-vi-tri-demo', 'Món nước', 'Trà tắc mật ong', 'Trà tắc mát lạnh, ít ngọt.', 'https://images.unsplash.com/photo-1499638670234-54e6a92b38f4?auto=format&fit=crop&w=800&q=80', 18000, 4, 0, 2, 67, 4.50
