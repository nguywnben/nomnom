...
const RECENT = [
  { label: 'Thanh toán hàng tuần', date: '13 thg 5', amount: 10250000, kind: 'out' },
  { label: 'Giao hàng #J21A', date: '14 thg 5', amount: 210000, kind: 'in' },
  { label: 'Tiền thưởng từ Mara C.', date: '14 thg 5', amount: 75000, kind: 'in' },
  { label: 'Giao hàng #J218', date: '14 thg 5', amount: 180000, kind: 'in' },
  { label: 'Tiền thưởng', date: '13 thg 5', amount: 125000, kind: 'in' },
];
  ...
  return (
    <div className="flex flex-col gap-base p-base">
      ...
      <Card variant="dark" padded>
        ...
        <div className="mt-sm nums text-display-lg leading-none">{formatVnd(total)}</div>
        <div className="mt-1 text-body-sm text-on-dark-soft">
          Hôm nay <span className="nums">{formatVnd(today)}</span> · {trips} chuyến
        </div>
            ...
            {range === 'week' ? (
              <BarChart data={data} margin={{ left: -20, right: 8, top: 5, bottom: 0 }}>
                ...
                <Tooltip
                  ...
                  formatter={(v) => [formatVnd(v), 'Thu nhập']}
                />
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <MiniStat label="Trung bình / chuyến" value={formatVnd(total / trips)} />
      </div>
            ...
            <Button
              onClick={() => {
                pushToast({
                  ...
                  message: `${formatVnd(Number(amount))} đang được xử lý. 1–2 ngày làm việc.`,
                });
                setWithdraw(false);
              }}
            >
              Rút {formatVnd(Number(amount))}
            </Button>
          </>
        }
      >
        <div className="space-y-sm">
          <Input
            ...
            aria-label="Số tiền rút VNĐ"
            hint={`Khả dụng: ${formatVnd(total)}`}
          />
          <Input
            value="Chase ··· 4823"
            readOnly
            leadingIcon="card"
            aria-label="Tài khoản ngân hàng nhận tiền"
